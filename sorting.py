...
from mergesort2 import merge_sort
...
def drawData(data, colorArray):
    canvas.delete("all") # to clear the canvas for next sorting
    canvas_height = 450
    canvas_width = 870
    ...
    for i, height in enumerate(normalised_data):
        ...
        y0 = canvas_height - height * 400 # multiply by 400 because y0 might be a very small value so in order to visually see the value we did so
...
algo_menu.current(0) # by Default it will be Bubble Sort
...
speedscale = Scale(root, from_= 0.1, to=5.1, resolution=0.2, length=200, digits=2, orient=HORIZONTAL, font=("arial", 14, "italic bold"), relief=GROOVE, bd=2, width=10)
